/** @file
 *
 *  Board init for the Rock3A platform
 *
  ...
  )
{
  ...
  /* Use sequential MAC addresses. Last byte is odd for GMAC1. */
  Hash[5] |= 1;
  ...
  )
{
  ...
  PmicWrite (PMIC_LDO5_ON_VSEL, 0x6c);  /* 1.8V/3.3V - vccio_sd */
  ...
  )
{
  ...
  /* Set GPIO0 PD5 (HUB_RST_GPIO1_A4_d) output high to take USB 2.0 hub out of reset */
  GpioPinSetDirection (0, GPIO_PIN_PD5, GPIO_PIN_OUTPUT);
  GpioPinWrite (0, GPIO_PIN_PD5, TRUE);

  /* Set GPIO0 PA5 (USB_OTG_EN) output high to take USB otg out of reset */
